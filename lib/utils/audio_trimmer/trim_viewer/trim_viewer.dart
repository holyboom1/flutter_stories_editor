...
import '../trim_viewer/trim_area_properties.dart';
import '../trimmer.dart';
...
class _TrimViewerState extends State<TrimViewer> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    widget.trimmer.eventStream.listen((TrimmerEvent event) async {
      if (event == TrimmerEvent.initialized) {
        ...
        final Duration maxAudioLength = widget.maxAudioLength;

        final double paddingFraction = widget.paddingFraction;
        final Duration trimAreaDuration = Duration(
            milliseconds: maxAudioLength.inMilliseconds +
                ((paddingFraction * maxAudioLength.inMilliseconds) * 2).toInt());

        final bool shouldScroll =
            trimAreaDuration <= totalDuration && maxAudioLength.compareTo(const Duration()) != 0;
  ...
  Widget build(BuildContext context) {
    final FixedTrimViewer fixedTrimViewer = FixedTrimViewer(
