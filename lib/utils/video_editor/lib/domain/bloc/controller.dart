...
import '../../domain/entities/ffmpeg_config.dart';
...
import '../../ui/trim/trim_slider.dart';
...
class VideoEditorController extends ChangeNotifier {
  ...
  void cropAspectRatio(double? value) {
    ...
    if (value != null) {
      final Size newSize = computeSizeWithRatio(videoDimension, value);
  ...
  void _videoListener() {
    final Duration position = videoPosition;
    if (position < _trimStart ||
        position >= _trimEnd - const Duration(milliseconds: 500)) {
      _video.seekTo(_trimStart);
      isLooped.value = isLooped.value + 1;
    }
  }
  ...
  String _getCrop() {
    ...
    final double enddx = videoWidth * maxCrop.dx;
    final double enddy = videoHeight * maxCrop.dy;
    final double startdx = videoWidth * minCrop.dx;
    final double startdy = videoHeight * minCrop.dy;
  ...
  Future<void> updateSelectedCover(CoverData selectedCover) async {
  ...
  Future<void> generateDefaultCoverThumbnail() async {
    final CoverData defaultCover = await generateSingleCoverThumbnail(
      file.path,
  ...
  String _getRotation() {
    final double count = rotation / 90;
    if (count <= 0 || count >= 4) return '';

    final List<String> transpose = <String>[];
